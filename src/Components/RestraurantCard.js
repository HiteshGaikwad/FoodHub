  ...
  // console.log(restraurant);
    return (
      <div className="flex flex-col gap-1 sm:gap-2 p-1">
        <img className="w-20 sm:w-80 h-14 sm:h-56 rounded-lg" src={IMG_CDN_URL+restraurant?.info?.cloudinaryImageId} alt={restraurant?.info?.name}/>
        <h2 className="sm:text-2xl text-xs font-bold sm:font-semibold" >{restraurant?.info?.name}</h2>
        <h4 className="sm:text-lg text-xs font-normal">{restraurant?.info?.cuisines.join(", ")}</h4>
