...
const Section=({title, discription, isVisible, setIsVisible})=>{
    ...
    return(
        <div className="my-4 shadow-md rounded-xl p-3 shadow-gray-300">
            ...
            {
               !isVisible ?<button className="text-lg italic text-blue-700 font-bold" onClick={()=>{
                setIsVisible(true);
            }}>Show</button> : <><button className="text-lg italic text-blue-700 font-bold" onClick={()=>{
                setIsVisible(false);
            }}>Hide</button>  
...
const About=()=>{
    ...
    <div className="flex flex-col items-center">
        ...
        <h1 className="sm:text-5xl text-3xl w-96 font-bold">What’s the FoodHub story?</h1>
        <p className="sm:text-xl text-lg max-w-xl font-bold line-h leading-7 mx-7">With love and support from consumers, FoodHub expanded far and wide, first through the entire city of Mumbai and then across the entire country. Today, FoodHub is the leading food ordering and delivery platform in India.</p>
        ...
        <div className="sm:m-7 my-10 border w-2/4">
